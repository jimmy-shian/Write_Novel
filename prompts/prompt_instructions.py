...
1. **創作黃金六階段依序推進**：世界觀 (`worldview`) -> 角色 (`characters`) -> 篇卷 (`volumes`) -> 章節骨架 (`volume_skeleton`) -> 全局伏筆對齊 (`foreshadowing_orchestration`) -> 詳細大綱 (`plot`) -> 正文寫作 (`writer`) -> 編輯姬精修 (`editor`)。
...
3. **合格即放行**：只要當前 `current_stage` 的內容品質合格，你**必須**輸出 `action: "CONTINUE"`，並將 `target` 設為下一個順序階段。例如：
   ...
   - 當 `current_stage` 為 `volume_skeleton`，合格後 `target` 應為 `foreshadowing_orchestration`。
   - 當 `current_stage` 為 `foreshadowing_orchestration`，合格後 `target` 應為 `plot` (詳細大綱)。
