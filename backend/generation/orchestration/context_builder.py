...
def build_generation_context(task: GenerationTaskRequest) -> Dict[str, Any]:
    ...
    memory_bundle: Dict[str, Any] = {"mode": task.context_mode, "data": {}}
    if task.target.chapter_index:
        if task.stage == "editor":
            chapter = db.get_latest_chapter(task.novel_id, task.target.chapter_index)
            memory_bundle["data"] = narrative_memory.build_editor_context_packet(
                task.novel_id,
                task.target.chapter_index,
                chapter.get("content", "") if chapter else "",
            )
        else:
            memory_bundle["data"] = narrative_memory.build_writer_memory_context(
                task.novel_id,
                task.target.chapter_index,
            )
