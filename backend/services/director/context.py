...
def _is_safe_director_memory(content):
    """Only feed prior Director messages back when they are valid decisions/results."""
    if not content:
        return False
    if "[展開檢視結果]" in content or "[評估結果]" in content:
        return True
    parsed = extract_json_block(content)
    if not isinstance(parsed, dict):
        return False
    action = str(parsed.get("action") or "").upper().strip()
    return action in VALID_DIRECTOR_ACTIONS
...
def build_director_conversation_context(novel_id, limit=1):
    ...
    for item in memory:
        ...
        if item.get("role") == "director" or item.get("message_type") == "director":
            if not _is_safe_director_memory(content):
                continue
        elif item.get("message_type") == "pipeline" and len(lines) >= 3:
            continue
...
def _snippet(text, head=600, tail=900):
    return narrative_memory.snippet_text(text, head=head, tail=tail)


def _tail(text, limit=1200):
    return narrative_memory.tail_text(text, limit=limit)
