...
from backend.common.utils import deep_merge_dict
...
def _foreshadowing_fields_changed(target_section, parsed_payload):
    if target_section in {"foreshadowing_seeds", "key_turning_points"}:
        return True
    if target_section != "worldbuilding" or not isinstance(parsed_payload, dict):
        return False
    return any(key in parsed_payload for key in ("foreshadowing_seeds", "key_turning_points"))


def _sync_foreshadowing_after_worldbuilding_patch(novel_id, target_section, parsed_payload):
    if not _foreshadowing_fields_changed(target_section, parsed_payload):
        return
    try:
        db.precompute_global_foreshadowing(novel_id)
        db.repair_foreshadowing_allocations(novel_id)
    ...
    try:
        ...
        print(f"[WARN] Failed to rebuild chapter memory after foreshadowing patch: {exc}")
        ...
        if target_section == "worldbuilding":
            ...
            for x in new_items:
                if isinstance(x, dict):
                    if x not in current_json[target_section]:
...
def validate_and_merge_incremental_patch(novel_id, target_section, action, payload, extra_params=None):
    ...
    try:
        if target_section in ["theme", "main_conflict", "worldview", "macro_outline", "foreshadowing_seeds", "key_turning_points", "multi_act_structure", "progressive_character_plan", "volumes", "worldbuilding"]:
            version = db.save_worldbuilding(novel_id, json.dumps(merged_data, ensure_ascii=False, indent=2), validate=False)
            ...
            _sync_foreshadowing_after_worldbuilding_patch(novel_id, target_section, parsed_payload)
            ...
            if isinstance(v, list) and len(v) > 0:
                ...
                if isinstance(curr_list, list):
                    ...
                    for x in v:
                        ...
                        elif isinstance(x, str) and x.strip() and x not in merged_list:
                            merged_list.append(x)
                    merged[k] = merged_list
                else:
                    merged[k] = v
...
def safe_worldbuilding_save(novel_id, new_json_data, source="unknown"):
    ...
    # Save
    try:
        version = db.save_worldbuilding(novel_id, json.dumps(merged, ensure_ascii=False, indent=2), validate=False)
        _sync_foreshadowing_after_worldbuilding_patch(novel_id, "worldbuilding", new_json_data)
