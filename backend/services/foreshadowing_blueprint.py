...
from backend.services.chapter_math import get_total_chapter_count, get_volume_chapter_range
...
def precompute_global_foreshadowing(novel_id):
    ...
    volumes = db.get_volumes(novel_id)
    if not volumes:
        from backend.config import MIN_CHAPTERS_PER_VOLUME, MIN_VOLUME_COUNT
