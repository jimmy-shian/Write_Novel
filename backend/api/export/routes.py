...
def api_export_novel(novel_id: str, format: str = "txt"):
    ...
    if format == "html":
        from backend.api.export.html_builder import build_novel_html

        content = build_novel_html(novel=novel, chapters=chapters, wb=wb, char=char, plot=plot)
        filename = f"{title}_便攜閱讀器.html"
        ...
        raise HTTPException(status_code=400, detail=f"Unsupported format: {format}. Supported formats are 'html' and 'txt'.")