...
2. 【角色數量與群像結構門檻】：
   - 角色總數至少達到 2~3 位以上（包含主角、主要反派/宿敵，以及關鍵配角或盟友）。
   - 若只有 1 位角色或缺乏主要反派，請指示角色設計師進行擴充（target: character_designer），補齊關鍵對抗勢力與人物群像，再行推進。
3. 角色關係網是否邏輯連貫，各角色利益與立場衝突是否清晰。
4. 檢查角色的心理深度 (Want/Need/Fatal Flaw)、語言人格 (speech_profile) 與成長弧線 (Arc) 是否完整。
5. 反派具備合理動機，配角擁有獨立成長線，豐富故事層次。
"""
...
9. **長程敘事與呼吸空間 (Story Engine 2.0)**：
   - 認可安靜章、過渡章、探索章與情感沉澱章（如 scene_function 為 recovery、transition、discovery、reflection）的健康存在。切勿要求每章都強行爆發衝突或生硬反轉。若本章定位為沉澱或信息探索且行文自然，應判定為通過，直接推進至 editor。
   - 審查因果衝突新穎度 (Conflict Novelty)：主角破局手段不得重複前卷或前章已用過的相同交鋒公式（如重複的扮豬吃虎、裝傻或公式化反轉）；破局必須承擔合理的代價或引發新代價。
...
6. **長程敘事節奏與呼吸空間**：對於呼吸章與日常推進章，潤色核心在於意象、氛圍與台詞次文本，切勿盲目添加戲劇性狗血衝突。審核合格後立即放行至下一章 writer。
