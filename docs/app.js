...
import { initServerConnection } from './ui/serverConnection.js';
import { initAutonomousDashboard } from './ui/autonomousDashboard.js';
...
async function runDirectorDecision(currentStage, providedUserPrompt = null, reviewContext = null, retryCount = 0) {
    ...
    return new Promise((resolve) => {
        ...
        streamAPI(
            ...
            // onError
            (err) => {
                if (err && err.includes('流水線正在執行中')) {
                    pipelineLockBusy = true;
                    console.warn('[Director] Pipeline lock busy during director decision, will retry with fallback.');
                    return;
                }
                responseText += `\n[總監連線錯誤: ${err}]`;
                throttledRenderMarkdown(streamTarget, responseText);
            ...
            async (success) => {
                if (pipelineLockBusy) {
                    ...
                    resolve({
                        ...
                        volume_index: null,
                        chapter_index: null,
                        reason: 'Pipeline lock race condition - auto-detect next stage'
                    });
                    return;
                }
...
function setupEventListeners() {
    ...
    // 4b. Toggle Novels List Collapse/Expand
    const novelsToggleBtn = document.getElementById('btn-toggle-novels');
    const novelsContainer = document.getElementById('novels-list-container');
    const novelsToggleIcon = document.getElementById('novels-toggle-icon');
    
    if (novelsToggleBtn && novelsContainer && novelsToggleIcon) {
        // Load saved state from localStorage
        const isCollapsed = localStorage.getItem('novelsListCollapsed') === 'true';
        if (isCollapsed) {
            novelsContainer.classList.add('collapsed');
            novelsToggleIcon.style.transform = 'rotate(-90deg)';
        }
        
        novelsToggleBtn.addEventListener('click', () => {
            const isNowCollapsed = novelsContainer.classList.toggle('collapsed');
            novelsToggleIcon.style.transform = isNowCollapsed ? 'rotate(-90deg)' : 'rotate(0deg)';
...
window.executeNextMissingStage = executeNextMissingStage;
