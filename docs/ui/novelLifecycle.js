...
export async function loadNovels() {
    try {
        ...
        // 若有小說且當前未選中，自動載入第 1 部小說
        if (state.novels && state.novels.length > 0 && !state.currentNovelId) {
            const savedNovelId = localStorage.getItem('currentNovelId');
            const targetNovel = state.novels.find(n => n.id === savedNovelId) || state.novels[0];
            if (targetNovel) {
                await loadNovelDetails(targetNovel.id);
            }
        }
    } catch (e) {
        console.error("Failed to load novels list", e);
    }
}
